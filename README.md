...
**Lean 4 形式化定理证明引擎 (v2.6 新增)**
- 整合 8 篇顶级学术成果，构建完整的 AI 辅助形式化数学证明系统
- **Hilbert 递归分解** (Apple, NeurIPS 2025)：miniF2F 94.7–99.2%，PutnamBench 70%
  - 四组件架构：非形式推理器 + 专业证明器 + 形式验证器 + 语义检索器
  - 证明失败时递归分解为子目标，逐个击破
- **COPRA 上下文学习** (NeurIPS 2023 / COLM 2024)：超越 ReProver 的 Pass@1
  - 栈式回溯搜索 + 失败字典（避免重复尝试已失败的策略）
  - 检索增强：自动查找相关引理辅助证明
- **MCTS 证明搜索** (DeepSeek-Prover-V1.5, ICLR 2025)：miniF2F 63.5%
  - UCB1 选择 + 策略扩展 + 值评估 + 反向传播
  - 迭代加深：先浅搜再深挖有前途的分支
- **STP 自我博弈** (ICML 2025)：miniF2F 65% Pass@3200（全证明 SOTA）
  - 猜想生成器：从已知定理变异产生新猜想（泛化/特化/类比/组合）
  - 难度自适应：追踪成功率，自动调节猜想难度目标
  - 涌现式数学发现：系统自主决定下一步该证明什么
- **Lean-STaR 交错思考** (ICLR 2025)：非形式推理与形式证明交替生成
- **Draft-Sketch-Prove** (ICLR 2023)：非形式证明 → 形式骨架 → 自动化补全
- **Pantograph 风格交互**：REPL 式 Lean 4 状态管理、策略执行
- **零公理哲学**：从 Lean 依赖类型论内核出发，逐层构建
  - Foundation → Elementary → Competition → Olympiad → Research 五级递进
  - 每级结果自动成为下一级的基础引理
- **文章形式化**：输入数学论文文本，输出完整 Lean 4 形式化文件
  - 自动提取定义/引理/定理/推论结构
  - 按依赖序逐个形式化并尝试证明
  - 未证明部分标记 sorry，报告覆盖率
- 配置：`lean_prover_enabled=True`

**CapabilityDAG — 自增长通用知识图谱 (v2.6 新增)**
- 统一的、领域无关的、社区可合并的能力有向无环图
- **核心思想**：每一条知识（数学证明、代码模式、架构决策、UI 规范、调试策略）都是图中一个节点，边是依赖关系。不区分"技能"和"数据"——能力就是数据，数据在使用中增长
- **内容寻址**：相同领域+相同内容 → 同一个 ID，天然支持跨用户/跨项目去重合并
- **验证多态**：
  - 数学：Lean 类型检查器（100% 确定）
  - 代码：测试通过率（高置信）
  - 设计/审美：LLM 评估 + 社区反馈（中置信）
  - 通用：使用次数 × 成功率的贝叶斯更新
- **自主生长**（STP 泛化版）：8 种变异策略（泛化/特化/组合/类比/逆否/分解/边界/抽象），系统自主决定生成什么新能力
- **社区合并**：多个 DAG 实例可以 merge，置信度取 max，使用次数累加，自动去重
