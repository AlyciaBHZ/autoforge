...
只需要 Python 3.11+ 和一个 Anthropic API Key。
...
安装完成后，直接在终端输入 `autoforge` 即可使用。首次运行会自动引导你填入 API Key、选择模型和预算。
...
AutoForge 生成的前端项目会自动附带 `DEPLOY_GUIDE.md`，包含：

1. **推送到 GitHub** — 完整的 git 命令
2. **Vercel 部署** — 自动检测框架，一键部署
3. **环境变量配置** — 每个变量的获取方式和说明
4. **域名设置** — 性价比方案推荐
...
- **Git**（推荐）— 项目生成时使用 worktree 隔离 — [下载](https://git-scm.com)
- **Docker**（可选）— 用于沙盒隔离执行
